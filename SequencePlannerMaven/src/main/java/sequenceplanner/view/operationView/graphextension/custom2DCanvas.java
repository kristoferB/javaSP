...
public class custom2DCanvas extends mxInteractiveCanvas {
   ...
   public static transient Font labelFont = new Font("Helvetica", Font.BOLD, 13);
   public static transient Font syncFont = new Font("Helvetica", Font.PLAIN, 10);
   // Shows boxes around text to find missalignments
   private boolean test = false;
   protected Stroke prePostOperation = new BasicStroke((float) (2 * scale), BasicStroke.CAP_BUTT,
         BasicStroke.JOIN_ROUND, 10.0f, new float[]{3.0f, 3.0f}, 0.0f);
   ...
   public void drawVertex(mxCellState state) {
      ...
      if (g != null && g.hitClip(x, y, w, h)) {
         ...
         if (c.isSOP()) {
            ...
            // Fast fix function to determine how long the alternative lines should be.
            if (!cell.isCollapsed()) {
                  ...
                  if (edge.getParent() == cell) {
                     try {
                        if (edge.getTerminal(true) == cell) {
                           double x1 = edgeState.getAbsolutePoint(0).getX();
                           topLeft = topLeft > x1 ? x1 : topLeft;
                           topRight = topRight < x1 ? x1 : topRight;
                        } else {
                           double x1 = edgeState.getAbsolutePoint(
                                 edgeState.getAbsolutePointCount() - 1).getX();
                           botLeft = botLeft > x1 ? x1 : botLeft;
                           botRight = botRight < x1 ? x1 : botRight;
                        }
                     } catch (Exception e) {
                        System.out.println("Error in calculation of alternative top-bottom");
                     }
                  }
               }
            }
            ...
            if (topRight > 0 || botRight > 0) {
               drawAlternative(x, y, w, h, (int) topLeft, (int) topRight, (int) botLeft, (int) botRight, s);
            } else {
               drawAlternative(x, y, w, h, x, x + w, x, x + w, s);
            }
         } else if (c.isArbitrary()) {

            Stroke s = new BasicStroke((float) (3 * scale), BasicStroke.CAP_BUTT,
                  BasicStroke.JOIN_ROUND, 10.0f, new float[]{10.0f, 6.0f}, 0.0f);
   ...
   private void drawOperation(int x, int y, int w, int h, mxCellState state) {
      ...
      OperationData value = (OperationData) ((mxCell) state.getCell()).getValue();
      SPGraph graph = ((SPGraph) state.getView().getGraph());
      ...
      g.setStroke(value.isPreoperation() || value.isPostoperation() ? prePostOperation : old);
      ...
      Font big = new Font(labelFont.getFamily(), labelFont.getStyle(), (int) Math.round(labelFont.getSize2D() * scale));
      Font small = new Font(syncFont.getFamily(), syncFont.getStyle(), (int) Math.round(syncFont.getSize2D() * scale));
      ...
      label.put(TextAttribute.SIZE, (int) Math.round(syncFont.getSize2D() * scale));
      ...
      // THIS IS A REAL SLOW SOLUTION, REMOVE IS EXPERIENCE PERFORMANCE PROBLEMS
      ...
      for (int i = 0; i < v.length(); i++) {
         if (Character.toString(v.charAt(i)).equals(Constansts.AND) || Character.toString(v.charAt(i)).equals(Constansts.OR)) {
            if (v.charAt(i - 1) == ' ' && v.charAt(i + 1) == ' ') {
               attText.addAttribute(TextAttribute.FOREGROUND, Color.BLUE, i, i + 1);
      ...
      Rectangle2D bounds = g.getFontMetrics().getStringBounds(v, g).getBounds();

      if (graph.OPTION_SHOW_STARTCONDITION && !v.isEmpty()) {
         g.setColor(Color.BLACK);

         if (test) {
            g.drawRect((int) (x + (w - bounds.getWidth()) / 2), y + start, (int) bounds.getWidth(), (int) bounds.getHeight());
         }

         start += g.getFontMetrics().getMaxAscent();
         g.setColor(Color.black);
         g.drawString(attText.getIterator(), (float) (x + (w - bounds.getWidth()) / 2), (float) (y + start - 2));
         start += 1 * scale;
         g.setColor(Color.GRAY);

         g.drawLine(x + inset.left, (y + start), x + w - inset.right, (y + start));
         start += 4 * scale;
      }
      ...
      if (graph.OPTION_SHOW_STOPCONDITION && !v.isEmpty()) {
         ...
         g.drawString(attText.getIterator(), (float) (x + (w - bounds.getWidth()) / 2), (float) (y + start));
      }
   }
   ...
   private void drawAlternative(int x, int y, int w, int h, int topLeft,
      ...
      g.drawLine((int) (scale * botLeft), y + h, (int) (scale * botRight), y + h);

      g.setStroke(old);
   }

   private void drawParallel(int x, int y, int w, int h, Stroke s, boolean top, boolean bottom, Color c) {
      ...
      if (bottom) {
         g.drawLine(x, y + h - (int) (7 * scale), x + w, y + h - (int) (7 * scale));
         g.drawLine(x, y + h - (int) (2 * scale), x + w, y + h - (int) (2 * scale));
      }
   ...
   private void drawGroup(int x, int y, int w, int h, mxCellState state) {
      ...
      Color c = mxUtils.getColor(state.getStyle(), mxConstants.STYLE_FILLCOLOR);
      g.setColor(new Color(c.getRed(), c.getGreen(), c.getBlue(), 60));

      OperationData data = (OperationData) ((mxCell) state.getCell()).getValue();
      ...
      g.drawString(v, (int) (x + w - 10 - r.getWidth()), (int) (y + r.getHeight()));


      Stroke old = g.getStroke();
      g.setStroke(data.isPreoperation() || data.isPostoperation() ? prePostOperation : new BasicStroke((float) 0.5));
      g.drawRoundRect(x, y, w, h, 20, 20);
      g.setStroke(old);
   }

   @Override
   public boolean hitSwimlaneContent(mxGraphComponent graphComponent, mxCellState swimlane, int x,
         int y) {
