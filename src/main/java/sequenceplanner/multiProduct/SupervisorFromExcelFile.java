...
public class SupervisorFromExcelFile {
    ...
    private ProbeForFlowerStructure mProbe;

    public SupervisorFromExcelFile(final String iExcelFilePath, final String iWmodFilePath) {
        mExcelFilePath = iExcelFilePath;
        mWmodFilePath = iWmodFilePath;

        mData = new OperationResourceDataStructure();
    }

    public ProbeForFlowerStructure getmProbe() {
        return mProbe;
    }

    public void setmProbe(ProbeForFlowerStructure mProbe) {
        this.mProbe = mProbe;
    }
    ...
    private boolean saveWMODFile() {
        if (mSModule == null || mWmodFilePath.length()<=1) {
            return false;
        }

        if (!mSModule.saveToWMODFile(mWmodFilePath)) {
            return false;
        }
...
//            System.out.println("Action: " + sega.getAction());
            ...
            mProbe.storeOperationAndTransition(op, sega);
            //-------------------------------------------------------------------
...
//            sega.setTransitionControllable(false);
...
//            System.out.println("Action: " + sega.getAction());
    ...
    private boolean initModule() {
        ...
        mSModule.setComment("Single flower for all transitions!");
    ...
    private boolean createVariablesForResources() {
        ...
        for (final OperationResourceDataStructure.Resource r : mData.mResourceSet) {
            ...
            //-------------------------------------------------------------------
            mProbe.addTomVariableSet(varName, lowerBound, upperBound, initialValue, markedValue);
            //-------------------------------------------------------------------
    ...
    private boolean parseOperationsFromExcelObject() {
        ...
        System.out.println("...finished with addition of operations!");
        return true;
    }
...
//            System.out.println(resourceInitValues.toString());
        ...
        System.out.println(" ...finished with addition of resources!");
