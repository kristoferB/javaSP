...
/**
 * Functions to be performed on a {@link SopNode}
 * @author patrik
 */
public interface ISopNodeToolbox {

    /**
     * Removes a {@link SopNode} and child nodes to this node.<br/>
     * E.g.:---------------<br/>
     ...
     */
    boolean removeNode(SopNode iNodeToRemove, SopNode iRootNode);
     ...
     */
    Set<OperationData> getOperations(SopNode iRootNode, boolean iGoDeep);
     ...
     */
    Map<OperationData, Map<ConditionsFromSopNode.ConditionType, Condition>> relationsToSelfContainedOperations(SopNode iRootNode);

    /**
     * Get all {@link SopNode} nodes recursively from sequences to iRootNode.<br/>
     * E.g.:---------------<br/>
     * node1: type operation, sequence set {node2}<br/>
     * node2: type parallel, sequence set {node3,node6}<br/>
     * node6: type operation, sequence set {}<br/>
     ...
     */
    Set<SopNode> getNodes(SopNode iRootNode, boolean iGoDeep);

    /**
     * To get the last {@link SopNode} in a successor sequence.<br/>
     * @param iNode start node
     * @return last {@link SopNode} in the successor sequence (this can be parameter iNode), or null if 1) parameter iNode ==null
     */
    SopNode getBottomSuccessor(SopNode iNode);

    /**
     * {@link SopNode} only has successor pointers.</br>
     ...
     * @return Predecessor as {@link SopNode} or null if no predecessor exists
     */
    SopNode getPredecessor(SopNode iSuccessorNode, SopNode iRootNode);

    /**
     * To get parent {@link SopNode} for parameter <p>iNode</p>.<br/>
     ...
     */
    SopNode getParentIfNodeIsInSequenceSet(SopNode iNode, SopNode iRoot);
}
