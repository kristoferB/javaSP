...
public class ConditionExpression extends ConditionElement implements Iterable<ConditionElement> {
    ...
    private boolean validateExpression(ConditionElement expressionRoot) {
        ...
        if (expressionRoot.getPreviousOperator() != null) {
            return false;
        }
        ConditionElement current = expressionRoot;
        while (current.hasNextOperator()) {
            ConditionOperator co = current.getNextOperator();
            if (!co.getPreviousElement().equals(current)) {
                return false;
            }
            if (!co.hasNextElement()) {
                return false;
            }
            current = co.getNextElement();
            if (!current.getPreviousOperator().equals(co)) {
                return false;
            }
        }
    ...
    private boolean isConnected(ConditionElement from, ConditionElement to) {
        if (from.getNextElement().equals(to) && to.getPreviousElement().equals(from) && from.getNextOperator().equals(to.getPreviousOperator())) {
            return true;
        } else {
            return false;
        }
    ...
    private List<ConditionElement> getFollowingElements(ConditionElement from) {
        ...
        if (!validateExpression(from)) {
            return list; //! included by Patrik 110612
        }
        list.add(from);
        ...
        public void remove() {
            ...
            if (current != null) {
                if (current.hasNextElement() && current.hasPreviousElement()) {
                    ...
                    if (current.getNextOperator().isOperationType(ConditionOperator.Type.OR) || current.getPreviousOperator().isOperationType(ConditionOperator.Type.OR)) {
